...
```
Server that polls AWS SQS for jobs, processes transcripts, and stores results in S3.
...
**Parameters**
* `request_type` (str, optional): Type of schema to retrieve for category type ("safety" or "summary")
...
**Returns**
* `dict | None`: Parsed message with job_id and transcript_s3_url, or None if invalid
...
**Notes**
Current storage path: `s3://{OUTPUT_BUCKET}/moderation-results/{job_id}/llm_{output_type}_result.json`
...
* `output_type` (str, optional): Type of result ("safety" or "summary"). Default: "safety"
...
* `json_schema` (dict[str, Any] | None, optional): JSON schema for guided output. Default: None
